...
class STFConvLayer(tf.keras.layers.Layer):
	...
	def call(self, inputs):
		...
		# def _initialize_stf_convs(self, 
		# 							c_in, 
		# 							c_out_total, 
		# 							kernel_size, 
		# 							conv_type='freq',
		...
		conv_kernel_dict = self._initialize_stf_convs(c_in,
													c_out, 
													kernel_size,
	...
	def _initialize_stf_convs(self, 
		...
		if conv_type == 'freq':
			# kernel_r = tf.get_variable('kernel_real', shape = [1, kernel_size, c_in, c_out],
			# 							initializer=tf.glorot_uniform_initializer())
			# kernel_i = tf.get_variable('kernel_imag', shape = [1, kernel_size, c_in, c_out],
			# 							initializer=tf.glorot_uniform_initializer())
			kernel_r = self.glorot_initializer(shape=[1, kernel_size, c_in, c_out])
			kernel_i = self.glorot_initializer(shape=[1, kernel_size, c_in, c_out])
		elif conv_type == 'time':
			# kernel = tf.get_variable('kernel', shape = [1, kernel_size, c_out, 2*(c_in+1)],
			# 							initializer=tf.glorot_uniform_initializer())
			kernel = self.glorot_initializer(shape=[1, kernel_size, c_out, 2*(c_in+1)])
		...
		if use_bias: 
			...
			# 							initializer=tf.glorot_uniform_initializer())
			bias_complex_r = self.zeros_initializer(shape=[c_out])
			bias_complex_i = self.zeros_initializer(shape=[c_out]) 
			bias_complex = tf.complex(bias_complex_r, bias_complex_i)
			return kernel_dict, bias_complex
		else:
			return kernel_dict

	def _initialize_stf_filters(self, 
								c_in, 
								c_out_total, 
								fft_list, 
								fft_n, 
